...
for frac_ind,this_frac in ipairs(size_frac) do 
	...
	m:add(nn.Dropout(.3)) -- layer 2

	m:add(nn.Linear(784,layer_sizes[1])) -- layer 3
	m:add(nn.PReLU())
	m:add(nn.Dropout(.3))
	...
	genOutput = true
	run_name = "DropoutNet0.3_PReLU_LearningRate0.05_LayerFrac"..this_frac.."_Epochs"..maxEpoch.."_trainSize"..trainSize.."_Layers"..layer_sizes[1].."-"..layer_sizes[2].."-"..layer_sizes[3] -- <-- UPDATE THIS LINE ON EACH RUN!!!
	if genOutput then
